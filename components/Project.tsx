...
export default function Project({ showAll = false, limit = 4 }: Props) {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 text-[#151515]">
        {projects.map((p, idx) => (
          <figure
            key={p.src + idx}
            className="relative overflow-hidden rounded-xs shadow-md"
            aria-label={p.role}
          >
            <Image
              src={p.src}
      ...
      {!showAll && (
        <div className="mt-8 text-center">
          <Link href="/projects" className="inline-block px-4 py-3 bg-[#0e0e0e] text-white rounded-sm shadow hover:brightness-110 transition uppercase tracking-normal font-medium text-sm">
            View more
          </Link>
        </div>
      )}
